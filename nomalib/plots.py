...
def plot_grid(g, sh=False, save=False, filename='grid', cells=False, ue=False, connect=False):
    ...
    plt.xlabel('Posição x [m]', fontsize=17)
    plt.ylabel('Posição y [m]', fontsize=16)
...
def plot_user_equipments(ue, style='sw', size=5):
    x = np.array([])
    y = np.array([])
    for u in ue:
...
def plot_cell_attenuation(site, sector=1, sh=False, save=False, filename='cell_att', den=const.MAP_D, shw_level=1):
    if site.bs.live:
        ...
        plt.xlabel('Posição x [m]', fontsize=17)
        plt.ylabel('Posição y [m]', fontsize=17)
        cbar = plt.colorbar()
        cbar.ax.set_ylabel('[dB]', fontsize=17)
...
def plot_doppler_filter(h, sh=False, save=False, filename='doppler_spread'):
    ...
    plt.xlabel('Frequência (GHz)',fontsize=17)
    plt.ylabel(r'$S(f)$',fontsize=19)
...
def plot_channel_gain(h, sh=False, save=False, filename='ch_gain'):
    ...
    plt.tick_params(labelsize=17)
    plt.xlabel('Tempo (ms)',fontsize=17)
    plt.ylabel('Ganho (dB)',fontsize=17)
    plt.grid(True)
...
def plot_path_loss(d=1, sh=False, save=False, filename='path_loss'):
    ...
    plt.tick_params(labelsize=17)
    plt.xlabel('Distância [m]', fontsize=17)
    plt.ylabel('Perda de Percurso [dB]', fontsize=17)
